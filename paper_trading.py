...
def ensure_paper_schema() -> None:
    with closing(get_connection()) as c:
        ...
        additions={"trade_side":"TEXT", "market_regime":"TEXT", "regime_confidence":"REAL", "regime_reasons":"TEXT"}
...
def record_delivered_trade(alert: dict[str, Any]) -> bool:
    ...
    with closing(get_connection()) as c:
        ...
        (signal_id,opened_at,stake,entry_price,category,alert_type,title,final_signal,ev_estimate,risk_score,
         trade_side,market_regime,regime_confidence,regime_reasons)
        VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",(
            ...
            float(alert.get("risk_score") or 0),side,regime["regime"],regime["confidence"],json.dumps(regime["reasons"],ensure_ascii=False)))
...
def get_paper_report()->dict[str,Any]:
    ...
    return {"total":total,"open":max(0,total-checkpoints["24ч"]["n"]),"checkpoints":checkpoints,"categories":categories,"regimes":regimes,
            "stake":float(getattr(config,"PAPER_TRADE_STAKE_USD",100)),"bank":float(getattr(config,"PAPER_TRADING_BANK_USD",10000)),"cost":float(getattr(config,"PAPER_TRADING_COST_PERCENT",1))}
...
def format_paper_report(r:dict[str,Any])->str:
    ...
    lines += ["","ℹ️ PUMP моделируется покупкой YES, DIP — покупкой NO. Market Regime работает в Shadow и не влияет на отправку."]
    return "\n".join(lines)
