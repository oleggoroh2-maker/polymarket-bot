...
from pilot_readiness_audit_v1 import (
...
async def handle_buttons(
    ...
    if text == "🔍 Сканировать":
    ...
    elif text == "📡 Rolling Edge":
        rolling = await asyncio.to_thread(get_rolling_edge_report, 1440, 10000)
    ...
    elif text == "🧬 Entry Features":
        for cp in (180, 360, 1440):
            report = await asyncio.to_thread(get_feature_report, cp)
            await update.message.reply_text(format_feature_report(report), reply_markup=ai_keyboard)
    ...
    elif text == "🎯 Stable Zone":
        report = await asyncio.to_thread(get_stable_zone_shadow_report)
        await update.message.reply_text(format_stable_zone_shadow_report(report), reply_markup=ai_keyboard)
        ...
        live_report = await asyncio.to_thread(get_live_performance_report)
